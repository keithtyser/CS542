...
class TwoLayerMLP(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        ###########################################################################
        y_mat = np.zeros_like(P)
        for index in range(N):
            y_mat[index][y[index]] = 1
        # output layer
        # [PLEASE IMPLEMENT] partial derivative of loss wrt. the logits (dL/dz)
        dscore = P - y_mat
        dW2 = np.dot(a1.T, dscore)/N  # partial derivative of loss wrt. W2
        db2 = np.mean(dscore, axis=0)     # partial derivation of loss wrt. b2

        # hidden layer
        dhidden = np.matmul(dscore, W2.T)
        if self.activation is 'relu':
            # [PLEASE IMPLEMENT]
